...
2. Для **Кандидата** по умолчанию используется **Windows loopback** — то есть
   всё, что реально воспроизводит ПК в звонке. Это **не выбор колонок/наушников**
   и **не второй микрофон**.
3. Если у вас установлен виртуальный loopback-кабель (например, VB-Cable /
   Stereo Mix), его можно выбрать в списке кандидата. Обычные физические
   микрофоны приложение больше не предлагает там специально, чтобы не было
   дублей HR под обеими ролями.
